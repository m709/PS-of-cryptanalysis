...
def lex_an(s):
    ...
    while((i<len(s))and(err==0)):
        if((i<len(s))and((s[i]>='a')and(s[i]<='z')or(s[i]>='A')and(s[i]<='Z')or(s[i]=='_'))):
            while((i<len(s))and((s[i]>='a')and(s[i]<='z')or(s[i]>='A')and(s[i]<='Z')or(s[i]=='_'))):
                buf=buf+s[i]; i=i+1;
            if((i<len(s))and(s[i]>='0')and(s[i]<='9')):
                while((i<len(s))and((s[i]>='a')and(s[i]<='z')or(s[i]>='A')and(s[i]<='Z')or(s[i]>='0')and(s[i]<='9')or(s[i]=='_'))):
                    buf=buf+s[i]; i=i+1;
                c=""
                if(i<len(s)):
                    c=""+s[i]; f=0;
                        ...
                        if kper>1 and s1.isnumeric() and s2.isnumeric():
                            ...
                            if n1>0 and max_gen>0:
                                ...
                                if len(re2) > 0:
                                    ...
                                    if flag2 == 0:
                                        textView2.insert("1.0","В уравнении есть синтаксические ошибки, или какая-то из переменных не определена на отрезке [-10;10], или в уравнении используется унарный минус, который не поддерживается программой.")
                                    else:
